...
/// A wrapper around `lsp::Location` that swaps out the LSP URI for ours.
...
impl TryFrom<lsp::Location> for Location {
    type Error = helix_core::uri::UrlConversionError;

    fn try_from(location: lsp::Location) -> Result<Self, Self::Error> {
        let uri = location.uri.try_into()?;
...
fn to_locations(definitions: Option<lsp::GotoDefinitionResponse>) -> Vec<Location> {
    match definitions {
        Some(lsp::GotoDefinitionResponse::Scalar(location)) => match location.try_into() {
            Ok(location) => vec![location],
            Err(_) => vec![],
        },
        Some(lsp::GotoDefinitionResponse::Array(locations)) => locations
            .into_iter()
            .flat_map(|location| location.try_into().ok())
            .collect(),
        Some(lsp::GotoDefinitionResponse::Link(locations)) => locations
            .into_iter()
            .flat_map(|location_link| {
                let uri = Uri::try_from(location_link.target_uri).ok()?;
